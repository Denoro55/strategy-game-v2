...
class Drawer {
  ...
  drawBackground(): void {
    const $ctx = this.$ctx;
    const { config: { stage: { cellSize, grid: { x, y } } }, utils } = this.game;
  ...
  drawGrid(): void {
    const { grid: { x, y } } = this.game.config.stage;
    const { viewOffset, stageCells } = this.game;
    const { cell: { color } } = this.config;
  ...
  drawHexon(x: number, y: number, options: IHexonOptions): void {
    const $ctx = this.$ctx;
    const { cell: { width, borderColor } } = this.config;
